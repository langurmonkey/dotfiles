...
c.aliases.update({'iu': 'open -t invidio.us/search?q='})
c.aliases.update({'pt': 'open -t tube.privacytools.io/search?search='})
c.aliases.update({'ya': 'open -t hooktube.com/results?search_query='})
c.aliases.update({'reddit': 'open -t reddit.com'})
c.aliases.update({'rd': 'open -t reddit.com'})
c.aliases.update({'tr': 'open -t deepl.com'})
...
config.bind(';V', 'spawn termite -e "youtube-dl --all-subs --output ~/temp/vid/yt/%(title)s.%(ext)s {url}"')
config.bind(';A', 'spawn termite -e "youtube-dl --extract-audio --audio-format mp3 --output ~/temp/audio/yt/%(title)s.%(ext)s {url}"')
